...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run dedicated Warrant Need Score diagnostics.")
    ...
    parser.add_argument("--aggregate-only", action="store_true", help="Skip training and only aggregate existing results.")
    parser.add_argument("--no-aggregate", action="store_true", help="Skip aggregation after training.")
...
def checkpoint_name(config: dict[str, Any]) -> str:
    return str(config.get("training", {}).get("checkpoint_name", "checkpoint.pt"))


def save_checkpoint_enabled(config: dict[str, Any]) -> bool:
    return bool(config.get("training", {}).get("save_checkpoint", False))
...
def to_cpu_state(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        return {key: to_cpu_state(item) for key, item in value.items()}
...
def write_checkpoint(
    *,
    ...
    payload = {
        "model": to_cpu_state(model.state_dict()),
...
def run_spec(config: dict[str, Any], spec, device: torch.device, out_dir: Path) -> dict[str, Any]:
    """Train and evaluate one WNS diagnostic variant.

    This intentionally does not reuse previously generated path-localization or
    edge-query-ablation outputs.  It reuses the model/runner implementation,
    but writes a fresh result under experiments/warrant_need_score/outputs.
    """
    ...
    iterator = tqdm(
        range(1, epochs + 1),
        desc=f"wns/{spec.domain}/{spec.model} {spec.variant} seed={spec.seed}",
        unit="epoch",
...
def aggregate(config_path: Path, output_root: Path) -> None:
    from experiments.warrant_need_score.agg import run_aggregation

    config = load_yaml(config_path)
    analysis_dir = output_root / "analysis"
    main_path = REPO_ROOT / str(config.get("inputs", {}).get("main_base_vs_warrant", ""))
    run_aggregation(
...
def main() -> None:
    ...
    output_root = REPO_ROOT / str(config.get("experiment", {}).get("output_root", "experiments/warrant_need_score/outputs"))
    results_path = output_root / "results.csv"
    ...
    if not args.aggregate_only:
        for spec in tqdm(specs, desc="warrant-need runs", unit="run", disable=not bool(config["training"].get("progress", True))):
            ...
            if (
                should_skip(out_dir, args.force)
                and checkpoint_ready
                and bool(config.get("experiment", {}).get("skip_completed", True))
            ):
